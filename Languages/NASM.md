...
# Метки
**Метка** - это вводимое программистом слово (идентификатор), с которым ассемблер ассоциирует некоторое число, чаще всего - адрес в памяти.

Если ассемблер видит метку перед командой (или перед директивой, выделяющей память под переменную), он воспринимает это как указание завести в своих внутренних таблицах новую метку и связать с ней соответствующий адрес, если же метка встречается в пара-метрах команды, то ассемблер вспоминает, какой именно адрес (или просто число) связано с данной меткой и подставляет этот адрес (число) вместо метки в команду.

Метки существуют в памяти самого ассемблера во время трансляции программы, некоторые продолжают существовать во время работы редактора связей, но готовая к исполнению программа, представленная в машинном коде, не будет содержать никаких меток, а только подставленные вместо них адреса.

`_start` - это специальная метка, которой помечается точка входа в программу, то есть то место в программе, куда операционная система должна передать управление после загрузки программы в оперативную память; иначе говоря, метка `_start` обозначает место, с которого начнётся выполнение программы.
...
![effective_address](./static/effective_address.png)
...
![bitwise_operations](./static/bitwise_operations.png)
...
![stack_frame](./static/stack_frame.png)
...
![macroprocessor_operation_diagram](./static/macroprocessor_operation_diagram.png)
...
### Макроповторения
Макропроцессор ассемблера NASM позволяет многократно (циклически) обрабатывать один и тот де фрагмент кода. Это достигается директивами `%rep` (от слова repetition) и `%endrep`. Директива `%rep` принимает один обязательный параметр, означающий количества повторений. Так же есть директива `%exitrep`, которая досрочно прекращает выполнение макроповторения.
```nasm
%assign n 50
%rep 100
       db n
...
### Многострочные макросы и локальные метки
Механизм **локальных меток в макросах** - механизм позволяющий локализовать метки для конкретного вызова макроса, нужен чтобы метки полученные вызовом одно и того же макроса в разных местах программы не конфликтовали друг с другом.

Чтобы его задействовать нужно начать имя метки с двух символов `%`.
```nasm
%%lp
```
Такая метка будет в каждом следующем макровызове заменяться новым (не повторяющимся) идентификатором.

### Макросы с переменным числом параметров
Ассемблер NASM позволяет задать для макроса переменное число параметров с помощью символа `-`.

Так например задается макрос, принимающий от одного до трех параметров:
```nasm
%macro mymacro 1-3
```
Задает макрос, допускающий произвольное количество параметров, не меньше двух:
```nasm
%macro museondmacro 2-*
```

Сами аргументы многострочного макроса в его теле обозначаются как `%1`, `%2` и т.д., но средства индексирования (то есть способа извлечь n-ый параметр, где n вычисляется уже во время макроподстановки) NASM не предусматривает.  
Проблему решает директива `%rotate`, позволяющая преобразовать параметры. Числовой параметр обозначает на сколько на сколько позиций следует сдвинуть номер параметров.
```nsm
%rotate 1
```
В данном случае сдвиг на число 1, так что параметр, ранее обозначавший %2, после этой директивы будет иметь обозначение %1. Обозначение %0 в ротации не участвует и никак не изменяется. Если указать отрицательный параметр, она произведет циклический сдвиг в обратном направлении.

### Макродирективы для работы со строками
Ассемблер NASM поддерживает две директивы, предназначенные для преобразования строк (строковых констант) во время макропроцессирования.
...
%substr var1 'abcd' 3
```
макропеременные var1, var2 и var3 получат значения 'а', 'b' и 'с' соответственно.

Всё это имеет смысл, только в случае, если в качестве аргумента директивы получают либо имя макропеременной, либо обозначение позиционного параметра в многострочном макросе.

Все макродирективы отрабатывают во время макропроцессирования (перед компиляцией, то есть задолго до выполнения программы), так что, разумеется, на момент соответствующих макроподстановок все используемые строки должны быть уже известны.
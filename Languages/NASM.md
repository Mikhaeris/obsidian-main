...
Возможны различные соглашения о том, какие из регистров подпрограмма имеет право "испортить", а какие должна после себя оставить в том виде, в котором они были на момент ее вызыва.
...
#### CDECL
Согласно этой конвенции подпрограмма имеет право портить EAX, EDX и ECX, а все остальные регистры общего назначения должна либо не трогать, либо сохранить и перд возвратом управления восстановить.  
Причины:
...
- **-o** - задет имя файла, в который следут записать результат трансляции.
- **-d** - используется для определения макросимвола.
- **-l** - генерация листинга, требуется указать имя файла.(листинг - подробный отчет ассемблера о проделан)
...
`%include` - макродиректива, которая приказывает сакропроцессору заменить ее саму на содержимое файла, указанного параметром директивы.
```nasm
%include "stud_io.inc"
```

### Многострочный макрос
Макрос может иметь несколько входных парметров.
...
Ассемблер NASM поддерживает соглашение о том, что любое обращение у памяти оформляется ос помощью квадратных скобок, если же их нет, то мы имеем дело с непосредственным или регистровым операндом.  
п